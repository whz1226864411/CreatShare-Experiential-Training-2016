<%@ page language="java" import="java.util.*" pageEncoding="UTF-8"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width,initial-scale=1.0">
    <title>running-togather</title>
    <link rel="stylesheet" type="text/css" href="<c:url value='/css/main.css'/>">
    <link rel="stylesheet" type="text/css" href="<c:url value='/css/normalize.css'/>">
</head>
<body>

<header>
    <div>

        <div class="search-bar"></div>
        <div class="nav">
            <ul>
                <li><a href="<c:url value='/jsps/rank.jsp'/>" >排行榜</a></li>
                <li><a href="<c:url value='/index.jsp'/>" >约跑</a></li>
                <li><a href="<c:url value='/AppiontmentServlet?method=loadMyInfo'/>" >我的资料</a></li>
                <li><a href="<c:url value='/jsps/creatshare.jsp'/>" >联系CreatShare</a></li>
            </ul>
        </div>
    </div>
</header>
<main id="main">
    <div >
        <div>
            <div class="user-info">
                <div><img src="<c:url value='/images/portrait-1.png'/>" alt="portrait-1"/></div>
                <div>
                    <span>${userinfo.uclass }</span>
                    <span>${userinfo.uname }</span>
                    <span>[约跑成功次数：${userinfo.success }次]</span>
                </div>

                <div>
                    <p>${userinfo.qianmin }</p>
                </div>
                <div>
                    <span><img src="<c:url value='/images/weixin.png'/>" alt="weixin"/></span>
                    <span>${userinfo.weixin }</span>
                    <span><img src="<c:url value='/images/phone.png'/>" alt="phone"/></span>
                    <span>${userinfo.tel }</span>
                </div>
                <c:if test="${!empty rid }"><a href="<c:url value='/AppiontmentServlet?method=yue&rid=${rid }'/>"><button>约</button></a></c:if>
            </div>
        </div>
    </div>


</main>
<footer></footer>


</body>
</html>